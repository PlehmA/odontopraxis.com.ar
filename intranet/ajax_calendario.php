...
switch ($_GET["accion"])
{
	case "listar_evento":
	{
		$query=$db->query("select * from ".CALENDARIO." where fecha='".$_GET["fecha"]."' and id_usuario = '".$_SESSION['idUsuario']."' order by id asc");
	...
	case "guardar_evento":
	{
		$query=$db->query("insert into ".CALENDARIO." (fecha,evento,id_usuario) values ('".$_GET["fecha"]."','".strip_tags($_GET["evento"])."','".$_SESSION['idUsuario']."')");
	...
	case "borrar_evento":
	{
		$query=$db->query("delete from ".CALENDARIO." where id='".$_GET["id"]."' and id_usuario = '".$_SESSION['idUsuario']."' limit 1");
	...
	case "generar_calendario":
	{
		...
		$eventos=array();
		$query=$db->query("select fecha,count(id) as total from ".CALENDARIO." where month(fecha)='".$fecha_calendario[1]."' and year(fecha)='".$fecha_calendario[0]."' and id_usuario='".$_SESSION['idUsuario']."' group by fecha");
		if ($fila=$query->fetch_array())
		{
			do
			{
