function checkEmail(str) {
	...
	function esFechaValida(anio, mes, dia)	{
		...
		var dias_mes = [31, 28, 31, 30, 31, 30,
						31, 31, 30, 31, 30, 31];
		
		if ((mes < 1) || (mes > 12))	// mes inválido
			return false;
		
		if (((anio % 4) == 0) && (mes == 2))	{	// es año bisiesto y es febrero
			if ((dia < 1) || (dia > 29))	// dia entre 1 y 29
				return false;
			return true;
		}
		
		if ((dia < 1) || (dia > dias_mes[mes-1]))	// día inválido
			return false;
	...
	function verificarFecha(str)	{
		...
		var anio = str.charAt(6) + str.charAt(7) + str.charAt(8) + str.charAt(9);
		var mes = str.charAt(3) + str.charAt(4);
		var dia = str.charAt(0) + str.charAt(1);
